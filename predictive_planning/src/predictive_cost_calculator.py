...
# Hard Safety Margin: 로봇 반경(0.3m) + 사람 반경(0.3m) = 0.6m
# 이 거리 이내에서는 확률과 관계없이 무조건 회피
HARD_SAFETY_DISTANCE = 0.6
...
class PredictiveCostCalculator:
    ...
    def _calculate_agent_cost(self, node_x: float, node_y: float,
        ...
        for k in range(num_samples):
            ...
            # [2025-11-30 추가] Hard Safety Margin 체크
            # 기존: 확률적 희석으로 인해 거리가 가까워도 비용이 낮을 수 있었음
            # 수정: 물리적 충돌 거리(0.6m) 이내면 무조건 무한대 비용 반환
            if dist < HARD_SAFETY_DISTANCE:
                return float('inf')  # 즉시 회피 필요
    ...
    def _calculate_agent_cost_detailed(self, node_x: float, node_y: float,
        ...
        for k in range(num_samples):
            ...
            # [2025-11-30 추가] Hard Safety Margin 체크 (detailed 버전)
            if dist < HARD_SAFETY_DISTANCE:
                return float('inf'), num_samples  # 모든 샘플이 위험으로 간주
